...
export async function synthesizeSpeech(text: string): Promise<Buffer> {
  ...
  const res = await fetch(`${API_BASE}/${voiceId}`, {
    ...
    body: JSON.stringify({
      ...
      // speed 1.2 — ElevenLabs' documented ceiling before delivery starts
      // sounding rushed/unnatural; bumped up from 1.15 per explicit
      // request for faster delivery.
      voice_settings: { stability: 0.45, similarity_boost: 0.8, style: 0.6, use_speaker_boost: true, speed: 1.2 },
    }),
  });
